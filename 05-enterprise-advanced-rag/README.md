...
- **The answer is extractive, not generated.** With no `OPENAI_API_KEY`, `src/generator.ts` runs
  ...
  Add a key (below) to see GPT-3.5-turbo actually compose an answer from the retrieved context.
...
The banner switches to `MODE: LIVE` and the final answer is generated by GPT-3.5-turbo from the top
...
src/
  ...
  generator.ts            LIVE (GPT-3.5-turbo) or MOCK (extractive) answers
