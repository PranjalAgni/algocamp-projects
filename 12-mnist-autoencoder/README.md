...
Natural next steps, if you want to keep going: shrink the bottleneck and watch how *little* it takes to break - but expect a surprise. Squeezing from 32 down to 8 or 4 barely moves reconstruction error (test MSE creeps from ~0.055 to ~0.075) and the three shapes stay cleanly separable in the latent space (a nearest-centroid classifier still hits ~85-95%). These digits are only three hugely different templates - a circle, a bar, and two stripes - so almost no information distinguishes them, and even a 2-number bottleneck holds it. Separability only collapses at `latentDim: 1` (~55%), where one number cannot keep three classes apart. The real lesson is that bottleneck size has to be measured against how much information the *data* actually carries, not chosen by intuition. From there: add Gaussian noise to the input but keep the clean image as the target to build a denoising autoencoder; or flag unseen shapes by their high reconstruction error (anomaly detection).
